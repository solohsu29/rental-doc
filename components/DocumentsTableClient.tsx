...
export default function DocumentsTableClient({ data }: DocumentsTableClientProps) {
  ...
  return (
    <>
      ...
      <Dialog open={showModal} onOpenChange={open => !open && setDeleteIds(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isBulk ? `Delete ${deleteIds?.length} Documents?` : "Delete Document?"}</DialogTitle>
            <DialogDescription>
              {isBulk
                ? `Are you sure you want to delete these ${deleteIds?.length} documents? This action cannot be undone.`
