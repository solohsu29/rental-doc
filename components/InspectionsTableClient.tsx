...
interface InspectionsTableClientProps {
  data: Inspection[];
}

export default function InspectionsTableClient({ data }: InspectionsTableClientProps) {
  ...
  const handleDelete = async () => {
    if (!deleteIds) return;
    setLoading(true);
    await fetch("/api/inspections/bulk-delete", {
      method: "POST",
  ...
  return (
    <>
      ...
      <Dialog open={showModal} onOpenChange={open => !open && setDeleteIds(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isBulk ? `Delete ${deleteIds?.length} Inspections?` : "Delete Inspection?"}</DialogTitle>
            <DialogDescription>
              {isBulk
                ? `Are you sure you want to delete these ${deleteIds?.length} inspections? This action cannot be undone.`
